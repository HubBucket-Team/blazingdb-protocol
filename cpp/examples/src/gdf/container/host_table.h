#ifndef HostGdfTable_H
#define HostGdfTable_H
 ...
 */
enum class HostTableColumnFormat
{
  AUTO,
  SCIENTIFIC,
  FIXED,
 ...
 * HostGdfTable<std::string, double, int, std::string> vt({"Name", "Weight", "Age", "Brother"});
 *
...
public:
   ...
   */
  HostGdfTable(std::vector<std::string> headers,
   ...
   */
  void setColumnFormat(const std::vector<HostTableColumnFormat> & column_format)
  {
...
protected:
  ...
  void print_each(TupleType && t, StreamType & stream, std::integral_constant<size_t, I>)
  {
    ...
    if (!_column_format.empty())
    {
      ...
      if (_column_format[I] == HostTableColumnFormat::SCIENTIFIC)
        stream << std::scientific;

      if (_column_format[I] == HostTableColumnFormat::FIXED)
        stream << std::fixed;

      if (_column_format[I] == HostTableColumnFormat::PERCENT)
  ...
  /// Column Format
  std::vector<HostTableColumnFormat> _column_format;
